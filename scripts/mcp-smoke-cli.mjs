...
const entry = path.join(root, "packages", "cli", "dist", "mcp", "index.js");
...
const payload =
  JSON.stringify({
    ...
    params: {
      ...
      capabilities: {},
      clientInfo: { name: "contextos-mcp-smoke-cli", version: "1.0.0-beta" },
    },
  }) + "\n";
...
function ok() {
  ...
  settled = true;
  child.kill();
  console.log("Bundled contextos-mcp initialize: ok");
  process.exit(0);
}

const timer = setTimeout(() => fail("Bundled MCP smoke timed out waiting for initialize."), 20_000);
